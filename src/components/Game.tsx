...
const Game = () => {
   ...
   const loadDailyScreenshot = React.useCallback(() => {
       ...
       const dayFolder = `${day}`;
       const dailyScreenshots = files.filter(file => file.startsWith(`${dayFolder}/`));
   ...
   const handleGuess = (input: string, date: Date, movieId: number) => {
       ...
       const isCorrect = movie && input.toLowerCase() === movie.title.toLowerCase();
       setGuesses([...guesses, {
           title: input,
           date,
           isCorrect,
               ...
               <div className="screenshot-thumbnails">
                   {Array(6).fill(null).map((_, index) => (
                       <div key={index} className="thumbnail-box">
                           {(gameStatus === 'won' || index <= highestIndexReached || revealedScreenshots.includes(screenshots[index])) ? (
                               <img
           ...
           <div className="guesses">
               <ul className='guesses-list'>
                   {guesses.map((guess, index) => (
                       ...
                       >
                           {guess.isCorrect ? <HiCheckCircle size={24} /> : <HiXCircle size={24} />}
           ...
           <div className='result'>
               {showResult && gameStatus === 'won' && (
                   <p className='result-correct'>
                       Correct! The movie is <strong>{movie.title} ({new Date(movie.release_date).getFullYear()})</strong>
                   </p>
               )}
               {showResult && gameStatus === 'lost' && (
                   <p className='result-wrong'>
                       Out of guesses! The correct answer was <strong>{movie.title} ({new Date(movie.release_date).getFullYear()})</strong>
                   </p>
               )}
           </div>

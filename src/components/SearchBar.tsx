...
const SearchBar = ({ onGuess, disabled }: {onGuess: (input: string, date: Date, movieId: number) => void, disabled: boolean}) => {
   ...
   const handleSubmit = (e: { preventDefault: () => void; }) => {
       e.preventDefault();
       if (results.length > 0) {
         handleSelectMovie(results[0].title, results[0].release_date, results[0].id);
       }
     };
    return (
     <div className="searchbar" ref={searchRef}>
       ...
       {results.length > 0 && (
         <ul className="results-list">
           {results.map((movie) => (
             <li key={movie.id} onClick={() => handleSelectMovie(movie.title, movie.release_date, movie.id)}>

export const REFERENCE_TIME = new Date("2024-12-11T22:40:00+08:00"); // 4:30 PM Malaysia time on Nov 19, 2024
...
export const getNextGameTime = () => {
  ...
  const periodsPassed = Math.floor(diffTime / (5 * 60 * 1000));

  const nextGameTime = new Date(
    REFERENCE_TIME.getTime() + (periodsPassed + 1) * 5 * 60 * 1000
  );
